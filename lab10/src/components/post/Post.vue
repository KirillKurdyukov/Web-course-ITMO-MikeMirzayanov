<template>
    <article>
        <a href="#" class="title" @click.prevent="onPost">{{ post.title }}</a>
        <div class="information">By {{ user.login }}, 16:42 02.06.2001</div>
...
export default {
    name: "Post",
    props: ["post", "countComments", "user"],
    methods: {
        onPost: function () {
            this.$root.$emit("onPost", this.post);
            this.$root.$emit("onChangePage", "PostWithComments");
        }
    }
}
</script>
